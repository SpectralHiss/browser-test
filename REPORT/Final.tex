...
\begin{abstract}
 ...
 (illustrated by the lack of proper CSP in IE) .
 Even worse than all these differences is when something is not standardised like the navigation policy or simply due to negligence.\\
 
 BrowserAudit takes a no nonsense approach and verifies what policy is effectively enforced in the browser by running tests. This kind of first principle 
 approach is appreciated and no doubt is a great tool for standards commitees or browser vendors to automatically keep track of how security is currently
 handled in browsers.\\
 
 This addition to BrowserAudit is dubbed Browseraudit Reloaded, it is the second personal project regarding testing client browser implmentation. It takes
 ...
 work allows to allow adding tests through an easy to use test adding interface and presents all the data to be used by the community 
 at large.\\
 It also offers a new application for developers at large to solve the exact pain of keeping track of all the steps one can take to ensure their website
 is safe from a lot of client-side attacks on the web.\
 By using the unique data we have and formally encoding our domain knowlege we have the kind of guarantees that will benefit the security of web at large 
 and make it more robust. \
 The other gain from doing it this way is the exercise of practical formalisation that is good to bridge the gap between academism and pragmatic\
 application and achieved a sort of practical theory and for the security researcher automate the finding of proper policy for combinations of browsers and as such
 could be a reasoning "assistance" tool.
